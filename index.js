...
raisedCard.innerHTML = `<h4> ${totalRaised.toLocaleString()}</h4> `

// grab number of games card and set its inner HTML
const gamesCard = document.getElementById("num-games");
const totalGames = GAMES_JSON.length
gamesCard.innerHTML = `<h5> ${totalGames}</h5>`
...
const numberOfUnfundedGames = GAMES_JSON.filter(games => games.pledged < games.goal).length;

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${totalRaised.toLocaleString()} has been raised for ${totalGames} games. Currently, ${numberOfUnfundedGames} game${numberOfUnfundedGames !== 1 ? 's remain' : ' remains'} unfunded. We need your help to fund these amazing games!`;
